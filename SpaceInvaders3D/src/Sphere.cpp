		...
		for (int j = 0; j <= slices; ++j) { //Paralelos
			...
			if (mode == MODEL_MODE::VERTEX_COLOR) {
				vertexC[i * (slices + 1) + j].position = ratio
					* glm::vec3(X, Y, Z);
				vertexC[i * (slices + 1) + j].color = glm::vec3(0.0f, 0.0f,
					0.0f);
			}
			else if (mode == MODEL_MODE::VERTEX_LIGHT_COLOR) {
				...
				vertexLT[i * (slices + 1) + j].position = ratio
					* glm::vec3(X, Y, Z);
				vertexLT[i * (slices + 1) + j].texture2 = glm::vec2(i, j);
...
void Sphere::load() {
	...
	if (mode == MODEL_MODE::VERTEX_COLOR) {
		...
		offset4 = sizeof(vertexLT[0].position) + sizeof(vertexLT[0].texture) + sizeof(vertexLT[0].texture2);
	}
	...
	//Fourth Attribute
	if (mode == MODEL_MODE::VERTEX_LIGHT_TEXTURE)
		glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, stride, (GLvoid*)offset4);
