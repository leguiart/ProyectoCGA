...
bool testSphereBoxIntersection(SBB sbb1, AABB aabb1) {
	float s, d = 0;
...
bool testBoxBoxIntersection(AABB aabb1, AABB aabb2) {
	if ((aabb1.min.x <= aabb2.max.x) && (aabb1.max.x >= aabb2.min.x))
		if ((aabb1.min.y <= aabb2.max.y) && (aabb1.min.y >= aabb2.max.y))
			if ((aabb1.min.z <= aabb2.max.z) && (aabb1.min.z >= aabb2.max.z))
				return true;
	return false;
}
